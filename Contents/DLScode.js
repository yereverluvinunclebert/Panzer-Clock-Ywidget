...
function getDateOfLast(dayName, monthName, year) {
    ...
    lastResult = last - (lastDay - day + 7) % 7;
...
//		gTheEnd = "";
	...
	print("%DST   numberOfMonth=" + getNumberOfMonth(startMonth));
	print("%DST   startDate=" + startDate);
	print("%DST   startHour=" + startHour);
	print("%DST   startMin=" + startMin);

	theStart = Date.UTC(startYear, getNumberOfMonth(startMonth), startDate, startHour, startMin, 0, 0);
	if (!useUTC) {
	...
	print("%DST   numberOfMonth=" + getNumberOfMonth(endMonth));
	print("%DST   endDate=" + endDate);
	print("%DST   endHour=" + endHour);
	print("%DST   endMin=" + endMin);

	theEnd = Date.UTC(endYear, getNumberOfMonth(endMonth), endDate, endHour, endMin, 0, 0);
	if (!useUTC) {
		theEnd -= theGMTOffset;
	}

	print("%DST   theEnd=	  " + theEnd);
	print("%DST   theEndUTC=" + (new Date(theEnd)).toUTCString());
